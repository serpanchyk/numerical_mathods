...
def method_evaluation(a, x, b, det, execution_time, input_id, pivoting_type, decimal_places):
    evaluation_dir = Path("..") / "evaluations"
    evaluation_dir.mkdir(parents=True, exist_ok=True)
    txt_path = evaluation_dir / f"evaluation{input_id}_{pivoting_type}.txt"
    ...
    with open(f"evaluations/evaluation{input_id}_{pivoting_type}.txt", 'w') as f:
        ...
        if x is not None:
            ...
            x_perturbed, det_perturbed = solve_gaussian(a, b_perturbed, type=pivoting_type, det_evaluation=False)
...
def save_solution(x, input_id, pivoting_type, decimal_places):
    output_dir = Path("..") / "outputs"
    output_dir.mkdir(parents=True, exist_ok=True)
    npz_path = output_dir / f"output{input_id}_{pivoting_type}.npz"
    txt_path = output_dir / f"output{input_id}_{pivoting_type}.txt"
